...
class Entity(LocalPlayer):
    ...
    def get_name(self, entity: int):
        player_info = self.mem.game_handle.read_uint(self.engine_ptr() + offsets.dwClientState_PlayerInfo)
                
        player_info_items = self.mem.game_handle.read_uint(self.mem.game_handle.read_uint(player_info + 0x40) + 0xC)
